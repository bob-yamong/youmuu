...
static int handle_enter_socket(const struct event_t *e, const char *task_info) {
    syslog(LOG_INFO, "Enter socket: %s, domain=%#x, type=%#x, protocol=%#x\n",
...
static int handle_exit_socket(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit socket: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit socket: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_socketpair(const struct event_t *e, const char *task_info) {
    syslog(LOG_INFO, "Enter socketpair: %s, domain=%#x, type=%#x, protocol=%#x\n",
            task_info, e->arg_s32[0], e->arg_s32[1], e->arg_s32[2]);
    return 0;
}
...
static int handle_exit_socketpair(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit socketpair: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else if (e->is_valid == true) {
        syslog(LOG_INFO, "Exit socketpair: success, %s, sv[0]=%d, sv[1]=%d, ret=%lld\n",
                task_info, e->arg_s32[0], e->arg_s32[1], e->ret);
    } else {
        syslog(LOG_INFO, "Exit socketpair: success but failed to read socket values, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_setsockopt(const struct event_t *e, const char *task_info) {
    if (e->is_valid == true) {
        syslog(LOG_INFO, "Enter setsockopt: %s, socktfd=%d, level=%#x, optname=%#x, optval=%u, optlen=%d\n",
                task_info, e->arg_s32[0], e->arg_s32[1], e->arg_s32[2], e->arg_u32[0], e->arg_s32[3]);
    } else {
        syslog(LOG_INFO, "Enter setsockopt: %s, socktfd=%d, level=%#x, optname=%#x, failed to read optval, optlen=%d\n",
...
static int handle_exit_setsockopt(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit setsockopt: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit setsockopt: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_getsockopt(const struct event_t *e, const char *task_info) {
    syslog(LOG_INFO, "Enter getsockopt: %s, socketfd=%d, level=%#x, optname=%#x\n",
            task_info, e->arg_s32[0], e->arg_s32[1], e->arg_s32[2]);
    return 0;
}
...
static int handle_exit_getsockopt(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit getsockopt: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else if (e->is_valid == true) {
        syslog(LOG_INFO, "Exit getsockopt: success, %s, optval=%u, optlen=%u, ret=%lld\n",
                task_info, e->arg_u32[0], e->arg_u32[1], e->ret);
    } else {
        syslog(LOG_INFO, "Exit getsockopt: success, %s, failed to read optval, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_getsockname(const struct event_t *e, const char *task_info) {
    syslog(LOG_INFO, "Enter getsockname: %s, socketfd=%d\n",
...
static int handle_exit_getsockname(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit getsockname: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else if (e->is_valid == true) {
        char ip_str[INET6_ADDRSTRLEN];
        get_ip_str(e, ip_str, sizeof(ip_str));
        syslog(LOG_INFO, "Exit getsockname: success, %s, address=%s:%u, ip_version=%s, ret=%lld\n",
                task_info, ip_str, e->port, 
                e->addr_family == AF_INET ? "IPv4" : e->addr_family == AF_INET6 ? "IPv6" : "Unknown", e->ret);
    } else {
        syslog(LOG_INFO, "Exit getsockname: success, %s, failed to read socket address info, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_getpeername(const struct event_t *e, const char *task_info) {
    syslog(LOG_INFO, "Enter getpeername: %s, socketfd=%d\n",
            task_info, e->arg_s32[0]);
    return 0;
}
...
static int handle_exit_getpeername(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit getpeername: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else if (e->is_valid == true) {
        char ip_str[INET6_ADDRSTRLEN];
        get_ip_str(e, ip_str, sizeof(ip_str));
        syslog(LOG_INFO, "Exit getpeername: success, %s, address=%s:%u, ip_version=%s, ret=%lld\n",
                task_info, ip_str, e->port, 
                e->addr_family == AF_INET ? "IPv4" : e->addr_family == AF_INET6 ? "IPv6" : "Unknown", e->ret);
    } else {
        syslog(LOG_INFO, "Exit getpeername: success, %s, failed to read socket address info, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_bind(const struct event_t *e, const char *task_info) {
    if (e->is_valid == true) {
        char ip_str[INET6_ADDRSTRLEN];
        get_ip_str(e, ip_str, sizeof(ip_str));
        syslog(LOG_INFO, "Enter bind: %s, socktfd=%d, addr=%s:%u, ip_version=%s\n",
                task_info, e->arg_s32[0], ip_str, e->port, 
                e->addr_family == AF_INET ? "IPv4" : e->addr_family == AF_INET6 ? "IPv6" : "Unknown");
    } else {
        syslog(LOG_INFO, "Enter bind: %s, socktfd=%d, failed to read address info\n",
                task_info, e->arg_s32[0]);
    }
    return 0;
}
...
static int handle_exit_bind(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit bind: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit bind: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_listen(const struct event_t *e, const char *task_info) {
    syslog(LOG_INFO, "Enter listen: %s, socktfd=%d, backlog=%d\n",
...
static int handle_exit_listen(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit listen: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit listen: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_accept(const struct event_t *e, const char *task_info) {
    syslog(LOG_INFO, "Enter accept: %s, socktfd=%d\n",
            task_info, e->arg_s32[0]);
    return 0;
}
...
static int handle_exit_accept(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit accept: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else if (e->is_null == true) {
        syslog(LOG_INFO, "Exit accept: success, %s, socket address info is not requested, ret=%lld\n",
                task_info, e->ret);
    } else if (e->is_valid == true) {
        char ip_str[INET6_ADDRSTRLEN];
        get_ip_str(e, ip_str, sizeof(ip_str));
        syslog(LOG_INFO, "Exit accept: success, %s, address=%s:%u, ip_version=%s, ret=%lld\n",
                task_info, ip_str, e->port, 
                e->addr_family == AF_INET ? "IPv4" : e->addr_family == AF_INET6 ? "IPv6" : "Unknown", e->ret);
    } else {
        syslog(LOG_INFO, "Exit accept: success, %s, failed to read socket address info, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_accept4(const struct event_t *e, const char *task_info) {
    syslog(LOG_INFO, "Enter accept4: %s, socktfd=%d, flags=%#x\n",
            task_info, e->arg_s32[0], e->arg_s32[1]);
    return 0;
}
...
static int handle_exit_accept4(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit accept4: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else if (e->is_null == true) {
        syslog(LOG_INFO, "Exit accept4: success, %s, socket address info is not requested, ret=%lld\n",
                task_info, e->ret);
    } else if (e->is_valid == true) {
        char ip_str[INET6_ADDRSTRLEN];
        get_ip_str(e, ip_str, sizeof(ip_str));
        syslog(LOG_INFO, "Exit accept4: success, %s, address=%s:%u, ip_version=%s, ret=%lld\n",
                task_info, ip_str, e->port, 
                e->addr_family == AF_INET ? "IPv4" : e->addr_family == AF_INET6 ? "IPv6" : "Unknown", e->ret);
    } else {
        syslog(LOG_INFO, "Exit accept4: success, %s, failed to read socket address info, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_connect(const struct event_t *e, const char *task_info) {
    if (e->is_valid == true) {
        char ip_str[INET6_ADDRSTRLEN];
        get_ip_str(e, ip_str, sizeof(ip_str));
        syslog(LOG_INFO, "Enter connect: %s, socktfd=%d, addr=%s:%u, ip_version=%s\n",
                task_info, e->arg_s32[0], ip_str, e->port, 
                e->addr_family == AF_INET ? "IPv4" : e->addr_family == AF_INET6 ? "IPv6" : "Unknown");
    } else {
        syslog(LOG_INFO, "Enter connect: %s, socktfd=%d, failed to read address info\n",
                task_info, e->arg_s32[0]);
    }
    return 0;
}
...
static int handle_exit_connect(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit connect: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit connect: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_shutdown(const struct event_t *e, const char *task_info) {
    syslog(LOG_INFO, "Enter shutdown: %s, socketfd=%d, how=%d\n",
            task_info, e->arg_s32[0], e->arg_s32[1]);
    return 0;
}
...
static int handle_exit_shutdown(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit shutdown: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit shutdown: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_recvfrom (const struct event_t *e, const char *task_info) {
    syslog(LOG_INFO, "Enter recvfrom: %s, socktfd=%d, msg_len=%llu, flags=%#x\n",
...
static int handle_exit_recvfrom(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit recvfrom: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else if (e->is_null == true) {
        syslog(LOG_INFO, "Exit recvfrom: success, %s, socket address info is not requested, ret=%lld\n",
                task_info, e->ret);
    } else if (e->is_valid == true) {
        char ip_str[INET6_ADDRSTRLEN];
        get_ip_str(e, ip_str, sizeof(ip_str));
        syslog(LOG_INFO, "Exit recvfrom: success, %s, src_addr=%s:%u, ip_version=%s, ret=%lld\n",
                task_info, ip_str, e->port, 
                e->addr_family == AF_INET ? "IPv4" : e->addr_family == AF_INET6 ? "IPv6" : "Unknown", e->ret);
    } else {
        syslog(LOG_INFO, "Exit recvfrom: success, %s, failed to read source info, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_recvmsg(const struct event_t *e, const char *task_info) {
    syslog(LOG_INFO, "Enter recvmsg: %s, socktfd=%d, msg_len=%llu, flags=%#x\n",
            task_info, e->arg_s32[0], e->arg_u64[0], e->arg_s32[1]);
    return 0;
}
...
static int handle_exit_recvmsg(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit recvmsg: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else if (e->is_valid == true) {
        char ip_str[INET6_ADDRSTRLEN];
        get_ip_str(e, ip_str, sizeof(ip_str));
        syslog(LOG_INFO, "Exit recvmsg: success, %s, src_addr=%s:%u, ip_version=%s, ret=%lld\n",
                task_info, ip_str, e->port, 
                e->addr_family == AF_INET ? "IPv4" : e->addr_family == AF_INET6 ? "IPv6" : "Unknown", e->ret);
    } else {
        syslog(LOG_INFO, "Exit recvmsg: success, %s, failed to read source info, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_recvmmsg(const struct event_t *e, const char *task_info) {
    syslog(LOG_INFO, "Enter recvmmsg: %s, socktfd=%d, vlen=%u, flags=%#x\n",
...
static int handle_exit_recvmmsg(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit recvmmsg: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit recvmmsg: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_sendto(const struct event_t *e, const char *task_info) {
    if (e->is_valid == true) {
        char ip_str[INET6_ADDRSTRLEN];
        get_ip_str(e, ip_str, sizeof(ip_str));
        syslog(LOG_INFO, "Enter sendto: %s, socktfd=%d, msg_len=%llu, flags=%#x, dest_addr=%s:%u, ip_version=%s\n",
                task_info, e->arg_s32[0], e->arg_u64[0], e->arg_s32[1], ip_str, e->port, 
                e->addr_family == AF_INET ? "IPv4" : e->addr_family == AF_INET6 ? "IPv6" : "Unknown");
    } else {
        syslog(LOG_INFO, "Enter sendto: %s, socktfd=%d, msg_len=%llu, flags=%#x, failed to read destination info\n",
                task_info, e->arg_s32[0], e->arg_u64[0], e->arg_s32[1]);
    }
    return 0;
}
...
static int handle_exit_sendto(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit sendto: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit sendto: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_sendmsg(const struct event_t *e, const char *task_info) {
    if (e->is_valid == true) {
        char ip_str[INET6_ADDRSTRLEN];
        get_ip_str(e, ip_str, sizeof(ip_str));
        syslog(LOG_INFO, "Enter sendmsg: %s, socktfd=%d, flags=%#x, dest_addr=%s:%u, ip_version=%s\n",
                task_info, e->arg_s32[0], e->arg_s32[1], ip_str, e->port, 
                e->addr_family == AF_INET ? "IPv4" : e->addr_family == AF_INET6 ? "IPv6" : "Unknown");
    } else {
        syslog(LOG_INFO, "Enter sendmsg: %s, socktfd=%d, flags=%#x, failed to read destination info\n",
                task_info, e->arg_s32[0], e->arg_s32[1]);
    }
    return 0;
}
...
static int handle_exit_sendmsg(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit sendmsg: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit sendmsg: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_sendmmsg(const struct event_t *e, const char *task_info) {
    syslog(LOG_INFO, "Enter sendmmsg: %s, socktfd=%d, vlen=%u, flags=%#x\n",
            task_info, e->arg_s32[0], e->arg_u32[0], e->arg_s32[1]);
    return 0;
}
...
static int handle_exit_sendmmsg(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit sendmmsg: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit sendmmsg: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_sethostname(const struct event_t *e, const char *task_info) {
    if (e->is_valid == true) {
        syslog(LOG_INFO, "Enter sethostname: %s, hostname=%s, len=%llu\n",
                task_info, e->arg_str, e->arg_u64[0]);
    } else {
        syslog(LOG_INFO, "Enter sethostname: %s, failed to read hostname, len=%llu\n",
...
static int handle_exit_sethostname(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit sethostname: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit sethostname: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_setdomainname(const struct event_t *e, const char *task_info) {
    if (e->is_valid == true) {
        syslog(LOG_INFO, "Enter setdomainname: %s, domainname=%s, len=%llu\n",
                task_info, e->arg_str, e->arg_u64[0]);
    } else {
        syslog(LOG_INFO, "Enter setdomainname: %s, failed to read domainname, len=%llu\n",
                task_info, e->arg_u64[0]);
    }
    return 0;
}
...
static int handle_exit_setdomainname(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit setdomainname: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit setdomainname: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_ioctl(const struct event_t *e, const char *task_info) {
    syslog(LOG_INFO, "Enter ioctl: %s, fd=%d, op=%llu\n",
...
static int handle_exit_ioctl(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit ioctl: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit ioctl: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_close(const struct event_t *e, const char *task_info) {
    syslog(LOG_INFO, "Enter close: %s, fd=%d\n",
            task_info, e->arg_s32[0]);
    return 0;
}
...
static int handle_exit_close(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit close: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit close: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_creat(const struct event_t *e, const char *task_info) {
    if (e->is_valid == true) {
        syslog(LOG_INFO, "Enter creat: %s, pathname=%s, mode=%#x\n",
                task_info, e->arg_str, e->arg_u32[0]);
    } else {
        syslog(LOG_INFO, "Enter creat: %s, failed to read pathname, mode=%#x\n",
...
static int handle_exit_creat(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit creat: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit creat: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_open(const struct event_t *e, const char *task_info) {
    if (e->is_valid == true) {
        syslog(LOG_INFO, "Enter open: %s, pathname=%s, flags=%#x, mode=%#x\n",
                task_info, e->arg_str, e->arg_s32[0], e->arg_u32[0]);
    } else {
        syslog(LOG_INFO, "Enter open: %s, failed to read pathname, mode=%#x\n",
                task_info, e->arg_u32[0]);
    }
    return 0;
}
...
static int handle_exit_open(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit open: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit open: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_openat(const struct event_t *e, const char *task_info) {
    if (e->is_valid == true) {
        syslog(LOG_INFO, "Enter openat: %s, dirfd=%d, pathname=%s, flags=%#x, mode=%#x\n",
                task_info, e->arg_s32[0], e->arg_str, e->arg_s32[1], e->arg_u32[0]);
    } else {
        syslog(LOG_INFO, "Enter openat: %s, dirfd=%d, failed to read pathname, flags=%#x, mode=%#x\n",
...
static int handle_exit_openat(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit openat: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit openat: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_openat2(const struct event_t *e, const char *task_info) {
    if (e->is_valid == true) {
        syslog(LOG_INFO, "Enter openat2: %s, dirfd=%d, pathname=%s, flags=%#llx, mode=%#llx, resolve=%llu, size=%llu\n",
                task_info, e->arg_s32[0], e->arg_str, e->arg_u64[1], e->arg_u64[2], e->arg_u64[3], e->arg_u64[0]);
    } else {
        syslog(LOG_INFO, "Enter openat2: %s, dirfd=%d, failed to read pathname, how, size=%llu\n",
                task_info, e->arg_s32[0], e->arg_u64[0]);
    }
    return 0;
}
...
static int handle_enter_name_to_handle_at(const struct event_t *e, const char *task_info) {
    if (e->is_valid == true) {
        syslog(LOG_INFO, "Enter name_to_handle_at: %s, dirfd=%d, pathname=%s, handle_bytes=%u, handle_type=%d, mount_id=%d, flags=%#x\n",
                task_info, e->arg_s32[0], e->arg_str, e->arg_u32[0], e->arg_s32[3], e->arg_s32[2], e->arg_s32[1]);
    } else {
        syslog(LOG_INFO, "Enter name_to_handle_at: %s, dirfd=%d, failed to read pathname, handle, mount_id, flags=%#x\n",
                task_info, e->arg_s32[0], e->arg_s32[1]);
    }
    return 0;
}
...
static int handle_exit_name_to_handle_at(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit name_to_handle_at: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit name_to_handle_at: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_open_by_handle_at(const struct event_t *e, const char *task_info) {
    if (e->is_valid == true) {
        syslog(LOG_INFO, "Enter open_by_handle_at: %s, mount_fd=%d, handle_bytes=%u, handle_type=%d, flags=%#x\n",
                task_info, e->arg_s32[0], e->arg_u32[0], e->arg_s32[2], e->arg_s32[1]);
    } else {
        syslog(LOG_INFO, "Enter open_by_handle_at: %s, mount_fd=%d, failed to read handle, flags=%#x\n",
                task_info, e->arg_s32[0], e->arg_s32[1]);
    }
    return 0;
}
...
static int handle_exit_open_by_handle_at(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit open_by_handle_at: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit open_by_handle_at: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_memfd_create(const struct event_t *e, const char *task_info) {
    if (e->is_valid == true) {
        syslog(LOG_INFO, "Enter memfd_create: %s, name=%s, flags=%#x\n",
                task_info, e->arg_str, e->arg_u32[0]);
    } else {
        syslog(LOG_INFO, "Enter memfd_create: %s, failed to read name, flags=%#x\n",
                task_info, e->arg_u32[0]);
    }
    return 0;
}
...
static int handle_exit_memfd_create(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit memfd_create: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit memfd_create: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_mknod(const struct event_t *e, const char *task_info) {
    if (e->is_valid == true) {
        syslog(LOG_INFO, "Enter mknod: %s, pathname=%s, mode=%#x, dev=%llu\n",
                task_info, e->arg_str, e->arg_u32[0], e->arg_u64[0]);
    } else {
        syslog(LOG_INFO, "Enter mknod: %s, failed to read pathname, mode=%#x, dev=%llu\n",
...
static int handle_exit_mknod(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit mknod: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit mknod: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_mknodat(const struct event_t *e, const char *task_info) {
    if (e->is_valid == true) {
        syslog(LOG_INFO, "Enter mknodat: %s, dirfd=%d, pathname=%s, mode=%#x, dev=%llu\n",
                task_info, e->arg_s32[0], e->arg_str, e->arg_u32[0], e->arg_u64[0]);
    } else {
        syslog(LOG_INFO, "Enter mknodat: %s, dirfd=%d, failed to read pathname, mode=%#x, dev=%llu\n",
...
static int handle_exit_mknodat(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit mknodat: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit mknodat: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_rename(const struct event_t *e, const char *task_info) {
    if (e->is_valid == true) {
        syslog(LOG_INFO, "Enter rename: %s, oldpath=%s, newpath=%s\n",
                task_info, e->arg_str, e->arg_str2);
    } else {
        syslog(LOG_INFO, "Enter rename: %s, failed to read oldpath, newpath\n",
                task_info);
    }
    return 0;
}
...
static int handle_exit_rename(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit rename: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit rename: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_renameat(const struct event_t *e, const char *task_info) {
    if (e->is_valid == true) {
        syslog(LOG_INFO, "Enter renameat: %s, olddirfd=%d, oldpath=%s, newdirfd=%d, newpath=%s\n",
                task_info, e->arg_s32[0], e->arg_str, e->arg_s32[1], e->arg_str2);
    } else {
        syslog(LOG_INFO, "Enter renameat: %s, olddirfd=%d, newdirfd=%d, failed to read oldpath, newpath\n",
                task_info, e->arg_s32[0], e->arg_s32[1]);
    }
    return 0;
}
...
static int handle_exit_renameat(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit renameat: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit renameat: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_renameat2(const struct event_t *e, const char *task_info) {
    if (e->is_valid == true) {
        syslog(LOG_INFO, "Enter renameat2: %s, olddirfd=%d, oldpath=%s, newdirfd=%d, newpath=%s, flags=%#x\n",
                task_info, e->arg_s32[0], e->arg_str, e->arg_s32[1], e->arg_str2, e->arg_s32[2]);
    } else {
        syslog(LOG_INFO, "Enter renameat2: %s, olddirfd=%d, newdirfd=%d, failed to read oldpath, newpath, flags=%#x\n",
                task_info, e->arg_s32[0], e->arg_s32[1], e->arg_s32[2]);
    }
    return 0;
}
...
static int handle_exit_renameat2(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit renameat2: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit renameat2: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_truncate(const struct event_t *e, const char *task_info) {
    if (e->is_valid == true) {
        syslog(LOG_INFO, "Enter truncate: %s, path=%s, length=%llu\n",
                task_info, e->arg_str, e->arg_u64[0]);
    } else {
        syslog(LOG_INFO, "Enter truncate: %s, failed to read path, length=%llu\n",
                task_info, e->arg_u64[0]);
    }
    return 0;
}
...
static int handle_exit_truncate(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit truncate: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit truncate: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_ftruncate(const struct event_t *e, const char *task_info) {
    syslog(LOG_INFO, "Enter ftruncate: %s, fd=%d, length=%llu\n",
            task_info, e->arg_s32[0], e->arg_u64[0]);
    return 0;
}
...
static int handle_exit_ftruncate(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit ftruncate: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit ftruncate: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_fallocate(const struct event_t *e, const char *task_info) {
    syslog(LOG_INFO, "Enter fallocate: %s, fd=%d, mode=%#x, offset=%llu, len=%llu\n",
...
static int handle_exit_fallocate(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit fallocate: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit fallocate: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_mkdir(const struct event_t *e, const char *task_info) {
    if (e->is_valid == true) {
        syslog(LOG_INFO, "Enter mkdir: %s, pathname=%s, mode=%#x\n",
                task_info, e->arg_str, e->arg_u32[0]);
    } else {
        syslog(LOG_INFO, "Enter mkdir: %s, failed to read pathname, mode=%#x\n",
                task_info, e->arg_u32[0]);
    }
    return 0;
}
...
static int handle_exit_mkdir(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit mkdir: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit mkdir: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_mkdirat(const struct event_t *e, const char *task_info) {
    if (e->is_valid == true) {
        syslog(LOG_INFO, "Enter mkdirat: %s, dirfd=%d, pathname=%s, mode=%#x\n",
                task_info, e->arg_s32[0], e->arg_str, e->arg_u32[0]);
    } else {
        syslog(LOG_INFO, "Enter mkdirat: %s, dirfd=%d, failed to read pathname, mode=%#x\n",
...
static int handle_exit_mkdirat(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit mkdirat: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit mkdirat: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_rmdir(const struct event_t *e, const char *task_info) {
    if (e->is_valid == true) {
        syslog(LOG_INFO, "Enter rmdir: %s, pathname=%s\n",
                task_info, e->arg_str);
    } else {
        syslog(LOG_INFO, "Enter rmdir: %s, failed to read pathname\n",
                task_info);
    }
    return 0;
}
...
static int handle_exit_rmdir(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit rmdir: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit rmdir: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_getcwd(const struct event_t *e, const char *task_info) {
    syslog(LOG_INFO, "Enter getcwd: %s, size=%llu\n",
            task_info, e->arg_u64[0]);
    return 0;
}
...
static int handle_exit_getcwd(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit getcwd: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit getcwd: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_chdir(const struct event_t *e, const char *task_info) {
    if (e->is_valid == true) {
        syslog(LOG_INFO, "Enter chdir: %s, pathname=%s\n",
                task_info, e->arg_str);
    } else {
        syslog(LOG_INFO, "Enter chdir: %s, failed to read pathname\n",
                task_info);
    }
    return 0;
}
...
static int handle_exit_chdir(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit chdir: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit chdir: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_fchdir(const struct event_t *e, const char *task_info) {
    syslog(LOG_INFO, "Enter fchdir: %s, fd=%d\n",
            task_info, e->arg_s32[0]);
    return 0;
}
...
static int handle_exit_fchdir(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit fchdir: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit fchdir: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_chroot(const struct event_t *e, const char *task_info) {
    if (e->is_valid == true) {
        syslog(LOG_INFO, "Enter chroot: %s, pathname=%s\n",
                task_info, e->arg_str);
    } else {
        syslog(LOG_INFO, "Enter chroot: %s, failed to read pathname\n",
                task_info);
    }
    return 0;
}
...
static int handle_exit_chroot(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit chroot: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit chroot: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_pivot_root(const struct event_t *e, const char *task_info) {
    if (e->is_valid == true) {
        syslog(LOG_INFO, "Enter pivot_root: %s, new_root=%s, put_old=%s\n",
                task_info, e->arg_str, e->arg_str2);
    } else {
        syslog(LOG_INFO, "Enter pivot_root: %s, failed to read new_root, put_old\n",
                task_info);
    }
    return 0;
}
...
static int handle_exit_pivot_root(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit pivot_root: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit pivot_root: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_getdents(const struct event_t *e, const char *task_info) {
    syslog(LOG_INFO, "Enter getdents: %s, fd=%u, count=%u\n",
...
static int handle_exit_getdents(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit getdents: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else if (e->is_valid == true) {
        syslog(LOG_INFO, "Exit getdents: success, %s, data=%llu, ret=%lld, \n",
                task_info, e->arg_u64[0], e->ret);
    } else {
        syslog(LOG_INFO, "Exit getdents: success, %s, failed to read data, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_getdents64(const struct event_t *e, const char *task_info) {
    syslog(LOG_INFO, "Enter getdents64: %s, fd=%d, count=%llu\n",
            task_info, e->arg_s32[0], e->arg_u64[0]);
    return 0;
}
...
static int handle_exit_getdents64(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit getdents64: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else if (e->is_valid == true) {
        syslog(LOG_INFO, "Exit getdents64: success, %s, data=%llu, ret=%lld, \n",
                task_info, e->arg_u64[0], e->ret);
    } else {
        syslog(LOG_INFO, "Exit getdents64: success, %s, failed to read data, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_link(const struct event_t *e, const char *task_info) {
    if (e->is_valid == true) {
        syslog(LOG_INFO, "Enter link: %s, oldpath=%s, newpath=%s\n",
                task_info, e->arg_str, e->arg_str2);
    } else {
        syslog(LOG_INFO, "Enter link: %s, failed to read oldpath, newpath\n",
                task_info);
    }
    return 0;
}
...
static int handle_exit_link(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit link: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit link: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_linkat(const struct event_t *e, const char *task_info) {
    if (e->is_valid == true) {
        syslog(LOG_INFO, "Enter linkat: %s, olddirfd=%d, oldpath=%s, newdirfd=%d, newpath=%s, flags=%#x\n",
                task_info, e->arg_s32[0], e->arg_str, e->arg_s32[1], e->arg_str2, e->arg_s32[2]);
    } else {
        syslog(LOG_INFO, "Enter linkat: %s, olddirfd=%d, newdirfd=%d, failed to read oldpath, newpath, flags=%#x\n",
                task_info, e->arg_s32[0], e->arg_s32[1], e->arg_s32[2]);
    }
    return 0;
}
...
static int handle_exit_linkat(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit linkat: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit linkat: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_symlink(const struct event_t *e, const char *task_info) {
    if (e->is_valid == true) {
        syslog(LOG_INFO, "Enter symlink: %s, target=%s, linkpath=%s\n",
                task_info, e->arg_str, e->arg_str2);
    } else {
        syslog(LOG_INFO, "Enter symlink: %s, failed to read target, linkpath\n",
                task_info);
    }
    return 0;
}
...
static int handle_exit_symlink(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit symlink: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit symlink: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_symlinkat(const struct event_t *e, const char *task_info) {
    if (e->is_valid == true) {
        syslog(LOG_INFO, "Enter symlinkat: %s, target=%s, newdirfd=%d, linkpath=%s\n",
                task_info, e->arg_str, e->arg_s32[0], e->arg_str2);
    } else {
        syslog(LOG_INFO, "Enter symlinkat: %s, newdirfd=%d, failed to read target, linkpath\n",
                task_info, e->arg_s32[0]);
    }
    return 0;
}
...
static int handle_exit_symlinkat(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit symlinkat: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit symlinkat: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_unlink(const struct event_t *e, const char *task_info) {
    if (e->is_valid == true) {
        syslog(LOG_INFO, "Enter unlink: %s, pathname=%s\n",
                task_info, e->arg_str);
    } else {
        syslog(LOG_INFO, "Enter unlink: %s, failed to read pathname\n",
                task_info);
    }
    return 0;
}
...
static int handle_exit_unlink(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit unlink: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit unlink: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_unlinkat(const struct event_t *e, const char *task_info) {
    if (e->is_valid == true) {
        syslog(LOG_INFO, "Enter unlinkat: %s, dirfd=%d, pathname=%s, flags=%#x\n",
                task_info, e->arg_s32[0], e->arg_str, e->arg_s32[1]);
    } else {
        syslog(LOG_INFO, "Enter unlinkat: %s, dirfd=%d, failed to read pathname, flags=%#x\n",
                task_info, e->arg_s32[0], e->arg_s32[1]);
    }
    return 0;
}
...
static int handle_exit_unlinkat(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit unlinkat: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit unlinkat: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_readlink(const struct event_t *e, const char *task_info) {
    if (e->is_valid == true) {
        syslog(LOG_INFO, "Enter readlink: %s, pathname=%s, size=%llu\n",
                task_info, e->arg_str, e->arg_u64[0]);
    } else {
        syslog(LOG_INFO, "Enter readlink: %s, failed to read pathname, size=%llu\n",
                task_info, e->arg_u64[0]);
    }
    return 0;
}
...
static int handle_exit_readlink(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit readlink: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit readlink: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_readlinkat(const struct event_t *e, const char *task_info) {
    if (e->is_valid == true) {
        syslog(LOG_INFO, "Enter readlinkat: %s, dirfd=%d, pathname=%s, size=%llu\n",
                task_info, e->arg_s32[0], e->arg_str, e->arg_u64[0]);
    } else {
        syslog(LOG_INFO, "Enter readlinkat: %s, dirfd=%d, failed to read pathname, size=%llu\n",
                task_info, e->arg_s32[0], e->arg_u64[0]);
    }
    return 0;
}
...
static int handle_exit_readlinkat(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit readlinkat: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit readlinkat: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_umask(const struct event_t *e, const char *task_info) {
    syslog(LOG_INFO, "Enter umask: %s, mask=%#x\n",
            task_info, e->arg_u32[0]);
    return 0;
}
...
static int handle_exit_umask(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit umask: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit umask: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_newstat(const struct event_t *e, const char *task_info) {
    if (e->is_valid == true) {
        syslog(LOG_INFO, "Enter newstat: %s, filename=%s\n",
                task_info, e->arg_str);
    } else {
        syslog(LOG_INFO, "Enter newstat: %s, failed to read filename\n",
                task_info);
    }
    return 0;
}
...
static int handle_exit_newstat(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit newstat: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit newstat: success, %s, ret=%lld, \n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_newlstat(const struct event_t *e, const char *task_info) {
    if (e->is_valid == true) {
        syslog(LOG_INFO, "Enter newlstat: %s, filename=%s\n",
                task_info, e->arg_str);
    } else {
        syslog(LOG_INFO, "Enter newlstat: %s, failed to read filename\n",
                task_info);
    }
    return 0;
}
...
static int handle_exit_newlstat(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit newlstat: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit newlstat: success, %s, ret=%lld, \n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_newfstat(const struct event_t *e, const char *task_info) {
    syslog(LOG_INFO, "Enter newfstat: %s, fd=%d\n",
            task_info, e->arg_s32[0]);
    return 0;
}
...
static int handle_exit_newfstat(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit newfstat: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit newfstat: success, %s, ret=%lld, \n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_newfstatat(const struct event_t *e, const char *task_info) {
    if (e->is_valid == true) {
        syslog(LOG_INFO, "Enter newfstatat: %s, dirfd=%d, pathname=%s, flags=%#x\n",
                task_info, e->arg_s32[0], e->arg_str, e->arg_s32[1]);
    } else {
        syslog(LOG_INFO, "Enter newfstatat: %s, dirfd=%d, failed to read pathname, flags=%#x\n",
                task_info, e->arg_s32[0], e->arg_s32[1]);
    }
    return 0;
}
...
static int handle_exit_newfstatat(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit newfstatat: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit newfstatat: success, %s, ret=%lld, \n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_statx(const struct event_t *e, const char *task_info) {
    if (e->is_valid == true) {
        syslog(LOG_INFO, "Enter statx: %s, dirfd=%d, pathname=%s, flags=%#x, mask=%#x\n",
                task_info, e->arg_s32[0], e->arg_str, e->arg_s32[1], e->arg_u32[0]);
    } else {
        syslog(LOG_INFO, "Enter statx: %s, dirfd=%d, failed to read pathname, flags=%#x, mask=%#x\n",
                task_info, e->arg_s32[0], e->arg_s32[1], e->arg_u32[0]);
    }
    return 0;
}
...
static int handle_exit_statx(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit statx: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit statx: success, %s, ret=%lld, \n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_statfs(const struct event_t *e, const char *task_info) {
    if (e->is_valid == true) {
        syslog(LOG_INFO, "Enter statfs: %s, pathname=%s\n",
                task_info, e->arg_str);
    } else {
        syslog(LOG_INFO, "Enter statfs: %s, failed to read pathname\n",
                task_info);
    }
    return 0;
}
...
static int handle_exit_statfs(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit statfs: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit statfs: success, %s, ret=%lld, \n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_fstatfs(const struct event_t *e, const char *task_info) {
    syslog(LOG_INFO, "Enter fstatfs: %s, fd=%d\n",
            task_info, e->arg_s32[0]);
    return 0;
}
...
static int handle_exit_fstatfs(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit fstatfs: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit fstatfs: success, %s, ret=%lld, \n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_chmod(const struct event_t *e, const char *task_info) {
    if (e->is_valid == true) {
        syslog(LOG_INFO, "Enter chmod: %s, pathname=%s, mode=%#x\n",
                task_info, e->arg_str, e->arg_u32[0]);
    } else {
        syslog(LOG_INFO, "Enter chmod: %s, failed to read pathname, mode=%#x\n",
                task_info, e->arg_u32[0]);
    }
    return 0;
}
...
static int handle_exit_chmod(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit chmod: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit chmod: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_fchmod(const struct event_t *e, const char *task_info) {
    syslog(LOG_INFO, "Enter fchmod: %s, fd=%d, mode=%#x\n",
            task_info, e->arg_s32[0], e->arg_u32[0]);
    return 0;
}
...
static int handle_exit_fchmod(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit fchmod: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit fchmod: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_fchmodat(const struct event_t *e, const char *task_info) {
    if (e->is_valid == true) {
        syslog(LOG_INFO, "Enter fchmodat: %s, dirfd=%d, pathname=%s, mode=%#x, flags=%#x\n",
                task_info, e->arg_s32[0], e->arg_str, e->arg_u32[0], e->arg_s32[1]);
    } else {
        syslog(LOG_INFO, "Enter fchmodat: %s, dirfd=%d, failed to read pathname, mode=%#x, flags=%#x\n",
                task_info, e->arg_s32[0], e->arg_u32[0], e->arg_s32[1]);
    }
    return 0;
}
...
static int handle_exit_fchmodat(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit fchmodat: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit fchmodat: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_chown(const struct event_t *e, const char *task_info) {
    if (e->is_valid == true) {
        syslog(LOG_INFO, "Enter chown: %s, pathname=%s, owner=%u, group=%u\n",
                task_info, e->arg_str, e->arg_u32[0], e->arg_u32[1]);
    } else {
        syslog(LOG_INFO, "Enter chown: %s, failed to read pathname, owner=%u, group=%u\n",
                task_info, e->arg_u32[0], e->arg_u32[1]);
    }
    return 0;
}
...
static int handle_exit_chown(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit chown: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit chown: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_lchown(const struct event_t *e, const char *task_info) {
    if (e->is_valid == true) {
        syslog(LOG_INFO, "Enter lchown: %s, pathname=%s, owner=%u, group=%u\n",
                task_info, e->arg_str, e->arg_u32[0], e->arg_u32[1]);
    } else {
        syslog(LOG_INFO, "Enter lchown: %s, failed to read pathname, owner=%u, group=%u\n",
                task_info, e->arg_u32[0], e->arg_u32[1]);
    }
    return 0;
}
...
static int handle_exit_lchown(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit lchown: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit lchown: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_fchown(const struct event_t *e, const char *task_info) {
    syslog(LOG_INFO, "Enter fchown: %s, fd=%d, owner=%u, group=%u\n",
...
static int handle_exit_fchown(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit fchown: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit fchown: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_fchownat(const struct event_t *e, const char *task_info) {
    if (e->is_valid == true) {
        syslog(LOG_INFO, "Enter fchownat: %s, dirfd=%d, pathname=%s, owner=%u, group=%u, flags=%#x\n",
                task_info, e->arg_s32[0], e->arg_str, e->arg_u32[0], e->arg_u32[1], e->arg_s32[1]);
    } else {
        syslog(LOG_INFO, "Enter fchownat: %s, dirfd=%d, failed to read pathname, owner=%u, group=%u, flags=%#x\n",
...
static int handle_exit_fchownat(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit fchownat: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit fchownat: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_access(const struct event_t *e, const char *task_info) {
    if (e->is_valid == true) {
        syslog(LOG_INFO, "Enter access: %s, pathname=%s, mode=%#x\n",
                task_info, e->arg_str, e->arg_s32[0]);
    } else {
        syslog(LOG_INFO, "Enter access: %s, failed to read pathname, mode=%#x\n",
                task_info, e->arg_s32[0]);
    }
    return 0;
}
...
static int handle_exit_access(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit access: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit access: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_faccessat(const struct event_t *e, const char *task_info) {
    if (e->is_valid == true) {
        syslog(LOG_INFO, "Enter faccessat: %s, dirfd=%d, pathname=%s, mode=%#x, flags=%#x\n",
                task_info, e->arg_s32[0], e->arg_str, e->arg_s32[1], e->arg_s32[2]);
    } else {
        syslog(LOG_INFO, "Enter faccessat: %s, dirfd=%d, failed to read pathname, mode=%#x, flags=%#x\n",
                task_info, e->arg_s32[0], e->arg_s32[1], e->arg_s32[2]);
    }
    return 0;
}
...
static int handle_exit_faccessat(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit faccessat: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit faccessat: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_fcntl(const struct event_t *e, const char *task_info) {
    syslog(LOG_INFO, "Enter fcntl: %s, fd=%d, cmd=%#llx, arg=%llu\n",
...
static int handle_exit_fcntl(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit fcntl: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit fcntl: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_dup(const struct event_t *e, const char *task_info) {
    syslog(LOG_INFO, "Enter dup: %s, oldfd=%d\n",
            task_info, e->arg_s32[0]);
    return 0;
}
...
static int handle_exit_dup(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit dup: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit dup: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_dup2(const struct event_t *e, const char *task_info) {
    syslog(LOG_INFO, "Enter dup2: %s, oldfd=%d, newfd=%d\n",
            task_info, e->arg_s32[0], e->arg_s32[1]);
    return 0;
}
...
static int handle_exit_dup2(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit dup2: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit dup2: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_dup3(const struct event_t *e, const char *task_info) {
    syslog(LOG_INFO, "Enter dup3: %s, oldfd=%d, newfd=%d, flags=%#x\n",
            task_info, e->arg_s32[0], e->arg_s32[1], e->arg_s32[2]);
    return 0;
}
...
static int handle_exit_dup3(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit dup3: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit dup3: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_flock(const struct event_t *e, const char *task_info) {
    syslog(LOG_INFO, "Enter flock: %s, fd=%d, operation=%d\n",
            task_info, e->arg_s32[0], e->arg_s32[1]);
    return 0;
}
...
static int handle_exit_flock(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit flock: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit flock: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_read(const struct event_t *e, const char *task_info) {
    syslog(LOG_INFO, "Enter read: %s, fd=%d, count=%llu\n",
            task_info, e->arg_s32[0], e->arg_u64[0]);
    return 0;
}
...
static int handle_exit_read(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit read: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit read: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_pread64(const struct event_t *e, const char *task_info) {
    syslog(LOG_INFO, "Enter pread: %s, fd=%d, count=%llu, offset=%lld\n",
...
static int handle_exit_pread64(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit pread: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit pread: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_readv(const struct event_t *e, const char *task_info) {
    syslog(LOG_INFO, "Enter readv: %s, fd=%d, iov_count=%d\n",
            task_info, e->arg_s32[0], e->arg_s32[1]);
    return 0;
}
...
static int handle_exit_readv(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit readv: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit readv: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_preadv(const struct event_t *e, const char *task_info) {
    syslog(LOG_INFO, "Enter preadv: %s, fd=%d, iov_count=%d, offset=%lld\n",
...
static int handle_exit_preadv(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit preadv: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit preadv: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_preadv2(const struct event_t *e, const char *task_info) {
    syslog(LOG_INFO, "Enter preadv2: %s, fd=%d, iov_count=%d, offset=%lld, flags=%#x\n",
...
static int handle_exit_preadv2(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit preadv2: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit preadv2: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_write(const struct event_t *e, const char *task_info) {
    syslog(LOG_INFO, "Enter write: %s, fd=%d, count=%llu\n",
            task_info, e->arg_s32[0], e->arg_u64[0]);
    return 0;
}
...
static int handle_exit_write(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit write: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit write: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_pwrite64(const struct event_t *e, const char *task_info) {
    syslog(LOG_INFO, "Enter pwrite: %s, fd=%d, count=%llu, offset=%lld\n",
            task_info, e->arg_s32[0], e->arg_u64[0], e->arg_s64[0]);
    return 0;
}
...
static int handle_exit_pwrite64(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit pwrite: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit pwrite: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_writev(const struct event_t *e, const char *task_info) {
    syslog(LOG_INFO, "Enter writev: %s, fd=%d, iov_count=%d\n",
            task_info, e->arg_s32[0], e->arg_s32[1]);
    return 0;
}
...
static int handle_exit_writev(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit writev: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit writev: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_pwritev(const struct event_t *e, const char *task_info) {
    syslog(LOG_INFO, "Enter pwritev: %s, fd=%d, iov_count=%d, offset=%lld\n",
            task_info, e->arg_s32[0], e->arg_s32[1], e->arg_s64[0]);
    return 0;
}
...
static int handle_exit_pwritev(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit pwritev: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit pwritev: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_pwritev2(const struct event_t *e, const char *task_info) {
    syslog(LOG_INFO, "Enter pwritev2: %s, fd=%d, iov_count=%d, offset=%lld, flags=%#x\n",
            task_info, e->arg_s32[0], e->arg_s32[1], e->arg_s64[0], e->arg_s32[2]);
    return 0;
}
...
static int handle_exit_pwritev2(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit pwritev2: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit pwritev2: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_lseek(const struct event_t *e, const char *task_info) {
    syslog(LOG_INFO, "Enter lseek: %s, fd=%d, offset=%lld, whence=%d\n",
...
static int handle_exit_lseek(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit lseek: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit lseek: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_sendfile64(const struct event_t *e, const char *task_info) {
    syslog(LOG_INFO, "Enter sendfile64: %s, out_fd=%d, in_fd=%d, offset=%lld, count=%llu\n",
...
static int handle_exit_sendfile64(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit sendfile64: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit sendfile64: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_inotify_init(const struct event_t *e, const char *task_info) {
    syslog(LOG_INFO, "Enter inotify_init: %s\n", task_info);
    return 0;
}

static int handle_exit_inotify_init(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit inotify_init: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit inotify_init: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_inotify_init1(const struct event_t *e, const char *task_info) {
    syslog(LOG_INFO, "Enter inotify_init1: %s, flags=%#x\n",
            task_info, e->arg_s32[0]);
    return 0;
}
...
static int handle_exit_inotify_init1(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit inotify_init1: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit inotify_init1: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_inotify_add_watch(const struct event_t *e, const char *task_info) {
    if (e->is_valid == true) {
        syslog(LOG_INFO, "Enter inotify_add_watch: %s, fd=%d, pathname=%s, mask=%#x\n",
                task_info, e->arg_s32[0], e->arg_str, e->arg_u32[0]);
    } else {
        syslog(LOG_INFO, "Enter inotify_add_watch: %s, fd=%d, failed to read pathname, mask=%#x\n",
                task_info, e->arg_s32[0], e->arg_u32[0]);
    }
    return 0;
}
...
static int handle_exit_inotify_add_watch(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit inotify_add_watch: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit inotify_add_watch: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_inotify_rm_watch(const struct event_t *e, const char *task_info) {
    syslog(LOG_INFO, "Enter inotify_rm_watch: %s, fd=%d, wd=%d\n",
            task_info, e->arg_s32[0], e->arg_s32[1]);
    return 0;
}
...
static int handle_exit_inotify_rm_watch(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit inotify_rm_watch: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit inotify_rm_watch: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_fanotify_init(const struct event_t *e, const char *task_info) {
    syslog(LOG_INFO, "Enter fanotify_init: %s, flags=%#x, event_f_flags=%#x\n",
            task_info, e->arg_s32[0], e->arg_s32[1]);
    return 0;
}
...
static int handle_exit_fanotify_init(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit fanotify_init: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit fanotify_init: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_fanotify_mark(const struct event_t *e, const char *task_info) {
    if (e->is_null == false) {
        if (e->is_valid == true) {
            syslog(LOG_INFO, "Enter fanotify_mark: %s, fanotify_fd=%d, flags=%#x, mask=%#llx, dirfd=%d, pathname=%s\n",
                    task_info, e->arg_s32[0], e->arg_u32[0], e->arg_u64[0], e->arg_s32[1], e->arg_str);
        } else {
            syslog(LOG_INFO, "Enter fanotify_mark: %s, fanotify_fd=%d, flags=%#x, mask=%#llx, dirfd=%d, failed to read pathname\n",
                    task_info, e->arg_s32[0], e->arg_u32[0], e->arg_u64[0], e->arg_s32[1]);
        }
    } else {
        syslog(LOG_INFO, "Enter fanotify_mark: %s, fanotify_fd=%d, flags=%#x, mask=%#llx, dirfd=%d, pathname is NULL (monitoring directory events)\n",
                task_info, e->arg_s32[0], e->arg_u32[0], e->arg_u64[0], e->arg_s32[1]);
    }
    return 0;
}
...
static int handle_exit_fanotify_mark(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit fanotify_mark: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit fanotify_mark: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_mount(const struct event_t *e, const char *task_info) {
    if (e->is_valid == true) {
        syslog(LOG_INFO, "Enter mount: %s, source=%s, target=%s, filesystemtype=%s, mountflags=%#llx\n",
                task_info, e->arg_str, e->arg_str2, e->filesystem_type, e->arg_u64[0]);
    } else {
        syslog(LOG_INFO, "Enter mount: %s, failed to read source, target, filesystemtype, mountflags=%#llx\n",
                task_info, e->arg_u64[0]);
    }
    return 0;
}
...
static int handle_exit_mount(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit mount: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit mount: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_umount(const struct event_t *e, const char *task_info) {
    if (e->is_valid == true) {
        syslog(LOG_INFO, "Enter umount: %s, target=%s, flags=%#x\n",
                task_info, e->arg_str, e->arg_s32[0]);
    } else {
        syslog(LOG_INFO, "Enter umount: %s, failed to read target, flags=%#x\n",
                task_info, e->arg_s32[0]);
    }
    return 0;
}
...
static int handle_exit_umount(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit umount: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit umount: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_move_mount(const struct event_t *e, const char *task_info) {
    if (e->is_valid == true) {
        syslog(LOG_INFO, "Enter move_mount: %s, from_fd=%d, from_pathname=%s, to_fd=%d, to_pathname=%s, flags=%#llx\n",
                task_info, e->arg_s32[0], e->arg_str, e->arg_s32[1], e->arg_str2, e->arg_u64[0]);
    } else {
        syslog(LOG_INFO, "Enter move_mount: %s, from_fd=%d, to_fd=%d, failed to read from_fd, from_pathname, to_fd, to_pathname, flags=%#llx\n",
...
static int handle_exit_move_mount(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit move_mount: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit move_mount: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_clone(const struct event_t *e, const char *task_info) {
    syslog(LOG_INFO, "Enter clone: %s, fn_ptr=%llu, flags=%#x\n",
...
static int handle_exit_clone(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit clone: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit clone: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_clone3(const struct event_t *e, const char *task_info) {
    if (e->is_valid == true) {
        syslog(LOG_INFO, "Enter clone3: %s, flags=%#llx, stack=%llu, stack_size=%llu, cgroup=%llu\n",
                task_info, e->arg_u64[0], e->arg_u64[1], e->arg_u64[2], e->arg_u64[3]);
    } else {
        syslog(LOG_INFO, "Enter clone3: %s, failed to read flags, stack, stack_size, cgroup\n",
                task_info);
    }
    return 0;
}
...
static int handle_exit_clone3(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit clone3: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit clone3: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_fork(const struct event_t *e, const char *task_info) {
    syslog(LOG_INFO, "Enter fork: %s\n", task_info);
    return 0;
}

static int handle_exit_fork(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit fork: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit fork: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_vfork(const struct event_t *e, const char *task_info) {
    syslog(LOG_INFO, "Enter vfork: %s\n", task_info);
    return 0;
}

static int handle_exit_vfork(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit vfork: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit vfork: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_execve(const struct event_t *e, const char *task_info) {
    if (e->is_valid == true) {
        syslog(LOG_INFO, "Enter execve: %s, pathname=%s\n",
                task_info, e->arg_str);
    } else {
        syslog(LOG_INFO, "Enter execve: %s, failed to read pathname\n",
                task_info);
    }
    return 0;
}
...
static int handle_exit_execve(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit execve: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit execve: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_execveat(const struct event_t *e, const char *task_info) {
    if (e->is_valid == true) {
        syslog(LOG_INFO, "Enter execveat: %s, dirfd=%d, pathname=%s, flags=%#x\n",
                task_info, e->arg_s32[0], e->arg_str, e->arg_s32[1]);
    } else {
        syslog(LOG_INFO, "Enter execveat: %s, dirfd=%d, failed to read pathname, flags=%#x\n",
                task_info, e->arg_s32[0], e->arg_s32[1]);
    }
    return 0;
}
...
static int handle_exit_execveat(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit execveat: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit execveat: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_exit(const struct event_t *e, const char *task_info) {
    syslog(LOG_INFO, "Enter exit: %s, status=%d\n",
            task_info, e->arg_s32[0]);
    return 0;
}
...
static int handle_exit_exit(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit exit: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit exit: success, %s\n", 
                task_info);
    }
    return 0;
}
...
static int handle_enter_exit_group(const struct event_t *e, const char *task_info) {
    syslog(LOG_INFO, "Enter exit_group: %s, status=%d\n",
            task_info, e->arg_s32[0]);
    return 0;
}
...
static int handle_exit_exit_group(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit exit_group: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit exit_group: success, %s\n", 
                task_info);
    }
    return 0;
}
...
static int handle_enter_wait4(const struct event_t *e, const char *task_info) {
    if (e->is_valid == true) {
        syslog(LOG_INFO, "Enter wait4: %s, pid=%u, status=%d, options=%#x\n",
                task_info, e->arg_u32[0], e->arg_s32[1], e->arg_s32[0]);
    } else {
        syslog(LOG_INFO, "Enter wait4: %s, pid=%u, failed to read status, options=%#x\n",
...
static int handle_exit_wait4(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit wait4: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit wait4: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_waitid(const struct event_t *e, const char *task_info) {
    if (e->is_valid == true) {
        syslog(LOG_INFO, "Enter waitid: %s, idtype=%d, pid=%u, si_signo=%d, si_code=%d, options=%#x\n",
                task_info, e->arg_s32[0], e->arg_u32[0], e->arg_s32[2], e->arg_s32[3], e->arg_s32[1]);
    } else {
        syslog(LOG_INFO, "Enter waitid: %s, idtype=%d, pid=%u, failed to siginfo, options=%#x\n",
                task_info, e->arg_s32[0], e->arg_u32[0], e->arg_s32[1]);
    }
    return 0;
}
...
static int handle_exit_waitid(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit waitid: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit waitid: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_getpid(const struct event_t *e, const char *task_info) {
    syslog(LOG_INFO, "Enter getpid: %s\n", task_info);
    return 0;
}

static int handle_exit_getpid(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit getpid: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit getpid: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_getppid(const struct event_t *e, const char *task_info) {
    syslog(LOG_INFO, "Enter getppid: %s\n", task_info);
    return 0;
}

static int handle_exit_getppid(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit getppid: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit getppid: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_gettid(const struct event_t *e, const char *task_info) {
    syslog(LOG_INFO, "Enter gettid: %s\n", task_info);
    return 0;
}

static int handle_exit_gettid(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit gettid: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit gettid: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_setsid(const struct event_t *e, const char *task_info) {
    syslog(LOG_INFO, "Enter setsid: %s\n", task_info);
    return 0;
}

static int handle_exit_setsid(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit setsid: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit setsid: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_getsid(const struct event_t *e, const char *task_info) {
    syslog(LOG_INFO, "Enter getsid: %s, pid=%u\n",
            task_info, e->arg_u32[0]);
    return 0;
}
...
static int handle_exit_getsid(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit getsid: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit getsid: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_setpgid(const struct event_t *e, const char *task_info) {
    syslog(LOG_INFO, "Enter setpgid: %s, pid=%u, pgid=%u\n",
            task_info, e->arg_u32[0], e->arg_u32[1]);
    return 0;
}
...
static int handle_exit_setpgid(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit setpgid: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit setpgid: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_getpgid(const struct event_t *e, const char *task_info) {
    syslog(LOG_INFO, "Enter getpgid: %s, pid=%u\n",
            task_info, e->arg_u32[0]);
    return 0;
}
...
static int handle_exit_getpgid(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit getpgid: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit getpgid: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_getpgrp(const struct event_t *e, const char *task_info) {
    syslog(LOG_INFO, "Enter getpgrp: %s\n", task_info);
    return 0;
}

static int handle_exit_getpgrp(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit getpgrp: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit getpgrp: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_setuid(const struct event_t *e, const char *task_info) {
    syslog(LOG_INFO, "Enter setreuid: %s, uid=%u\n",
            task_info, e->arg_u32[0]);
    return 0;
}
...
static int handle_exit_setuid(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit setreuid: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit setreuid: success, %s, ret=%lld\n", 
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_getuid(const struct event_t *e, const char *task_info) {
    syslog(LOG_INFO, "Enter getuid: %s\n", task_info);
    return 0;
}

static int handle_exit_getuid(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit getuid: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit getuid: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_setgid(const struct event_t *e, const char *task_info) {
    syslog(LOG_INFO, "Enter setregid: %s, gid=%u\n",
            task_info, e->arg_u32[0]);
    return 0;
}
...
static int handle_exit_setgid(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit setregid: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit setregid: success, %s, ret=%lld\n", 
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_getgid(const struct event_t *e, const char *task_info) {
    syslog(LOG_INFO, "Enter getgid: %s\n", task_info);
    return 0;
}

static int handle_exit_getgid(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit getgid: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit getgid: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_setresuid(const struct event_t *e, const char *task_info) {
    syslog(LOG_INFO, "Enter setresuid: %s, ruid=%u, euid=%u, suid=%u\n",
...
static int handle_exit_setresuid(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit setresuid: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit setresuid: success, %s, ret=%lld\n", 
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_getresuid(const struct event_t *e, const char *task_info) {
    syslog(LOG_INFO, "Enter getresuid: %s\n", task_info);
    return 0;
}

static int handle_exit_getresuid(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit getresuid: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else if (e->is_valid == true) {
        syslog(LOG_INFO, "Exit getresuid: success, %s, ruid=%u, euid=%u, suid=%u, ret=%lld\n",
                task_info, e->arg_u32[0], e->arg_u32[1], e->arg_u32[2], e->ret);
    } else {
        syslog(LOG_INFO, "Exit getresuid: success, %s, failed to read uid values, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_setresgid(const struct event_t *e, const char *task_info) {
    syslog(LOG_INFO, "Enter setresgid: %s, rgid=%u, egid=%u, sgid=%u\n",
            task_info, e->arg_u32[0], e->arg_u32[1], e->arg_u32[2]);
    return 0;
}
...
static int handle_exit_setresgid(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit setresgid: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit setresgid: success, %s, ret=%lld\n", 
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_getresgid(const struct event_t *e, const char *task_info) {
    syslog(LOG_INFO, "Enter getresgid: %s\n", task_info);
    return 0;
}

static int handle_exit_getresgid(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit getresgid: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else if (e->is_valid == true) {
        syslog(LOG_INFO, "Exit getresgid: success, %s, rgid=%u, egid=%u, sgid=%u, ret=%lld\n",
                task_info, e->arg_u32[0], e->arg_u32[1], e->arg_u32[2], e->ret);
    } else {
        syslog(LOG_INFO, "Exit getresgid: success, %s, failed to read gid values, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_setreuid(const struct event_t *e, const char *task_info) {
    syslog(LOG_INFO, "Enter setreuid: %s, ruid=%u, euid=%u\n",
            task_info, e->arg_u32[0], e->arg_u32[1]);
    return 0;
}
...
static int handle_enter_setregid(const struct event_t *e, const char *task_info) {
    syslog(LOG_INFO, "Enter setregid: %s, rgid=%u, egid=%u\n",
            task_info, e->arg_u32[0], e->arg_u32[1]);
    return 0;
}
...
static int handle_enter_geteuid(const struct event_t *e, const char *task_info) {
    syslog(LOG_INFO, "Enter geteuid: %s\n", task_info);
    return 0;
}

static int handle_exit_geteuid(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit geteuid: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit geteuid: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_getegid(const struct event_t *e, const char *task_info) {
    syslog(LOG_INFO, "Enter getegid: %s\n", task_info);
    return 0;
}

static int handle_exit_getegid(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit getegid: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit getegid: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_setgroups(const struct event_t *e, const char *task_info) {
    syslog(LOG_INFO, "Enter setgroups: %s, size=%llu\n",
            task_info, e->arg_u64[0]);
    return 0;
}
...
static int handle_exit_setgroups(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit setgroups: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit setgroups: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_getgroups(const struct event_t *e, const char *task_info) {
    syslog(LOG_INFO, "Enter getgroups: %s, size=%d\n",
            task_info, e->arg_s32[0]);
    return 0;
}
...
static int handle_exit_getgroups(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit getgroups: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit getgroups: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_setns(const struct event_t *e, const char *task_info) {
    syslog(LOG_INFO, "Enter setns: %s, fd=%d, nstype=%d\n",
            task_info, e->arg_s32[0], e->arg_s32[1]);
    return 0;
}
...
static int handle_exit_setns(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit setns: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit setns: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_setrlimit(const struct event_t *e, const char *task_info) {
    if (e->is_valid == true) {
        syslog(LOG_INFO, "Enter setrlimit: %s, resource=%d, rlim_cur=%llu, rlim_max=%llu\n",
                task_info, e->arg_s32[0], e->arg_u64[0], e->arg_u64[1]);
    } else {
        syslog(LOG_INFO, "Enter setrlimit: %s, resource=%d, failed to read rlim_cur, rlim_max\n",
                task_info, e->arg_s32[0]);
    }
    return 0;
}
...
static int handle_exit_setrlimit(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit setrlimit: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit setrlimit: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_getrlimit(const struct event_t *e, const char *task_info) {
    syslog(LOG_INFO, "Enter getrlimit: %s, resource=%d\n",
            task_info, e->arg_s32[0]);
    return 0;
}
...
static int handle_exit_getrlimit(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit getrlimit: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else if (e->is_valid == true) {
        syslog(LOG_INFO, "Exit getrlimit: success, %s, rlim_cur=%llu, rlim_max=%llu, ret=%lld\n",
                task_info, e->arg_u64[0], e->arg_u64[1], e->ret);
    } else {
        syslog(LOG_INFO, "Exit getrlimit: success, %s, failed to read rlimit info, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_prlimit64(const struct event_t *e, const char *task_info) {
    syslog(LOG_INFO, "Enter prlimit64: %s, pid=%u, resource=%d\n",
            task_info, e->arg_u32[0], e->arg_s32[0]);
    return 0;
}
...
static int handle_exit_prlimit64(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit prlimit64: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit prlimit64: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_getrusage(const struct event_t *e, const char *task_info) {
    syslog(LOG_INFO, "Enter getrusage: %s, who=%d\n",
            task_info, e->arg_s32[0]);
    return 0;
}
...
static int handle_exit_getrusage(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit getrusage: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    }else {
        syslog(LOG_INFO, "Exit getrusage: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_setpriority(const struct event_t *e, const char *task_info) {
    syslog(LOG_INFO, "Enter setpriority: %s, which=%d, who=%u, priority=%d\n",
            task_info, e->arg_s32[0], e->arg_u32[0], e->arg_s32[1]);
    return 0;
}
...
static int handle_exit_setpriority(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit setpriority: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit setpriority: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_getpriority(const struct event_t *e, const char *task_info) {
    syslog(LOG_INFO, "Enter getpriority: %s, which=%d, who=%u\n",
            task_info, e->arg_s32[0], e->arg_u32[0]);
    return 0;
}
...
static int handle_exit_getpriority(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit getpriority: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit getpriority: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_ioprio_set(const struct event_t *e, const char *task_info) {
    syslog(LOG_INFO, "Enter ioprio_set: %s, which=%d, who=%d, ioprio=%d\n",
            task_info, e->arg_s32[0], e->arg_s32[1], e->arg_s32[2]);
    return 0;
}
...
static int handle_exit_ioprio_set(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit ioprio_set: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit ioprio_set: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_ioprio_get(const struct event_t *e, const char *task_info) {
    syslog(LOG_INFO, "Enter ioprio_get: %s, which=%d, who=%d\n",
            task_info, e->arg_s32[0], e->arg_s32[1]);
    return 0;
}
...
static int handle_exit_ioprio_get(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit ioprio_get: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit ioprio_get: success, %s, ioprio=%d, ret=%lld\n",
...
static int handle_enter_mmap(const struct event_t *e, const char *task_info) {
    syslog(LOG_INFO, "Enter mmap: %s, addr=%#llx, length=%llu, prot=%#x, flags=%#x, fd=%d, offset=%llu\n",
...
static int handle_exit_mmap(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit mmap: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit mmap: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_mprotect(const struct event_t *e, const char *task_info) {
    syslog(LOG_INFO, "Enter mprotect: %s, len=%llu, prot=%#x\n",
            task_info, e->arg_u64[0], e->arg_s32[0]);
    return 0;
}
...
static int handle_exit_mprotect(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit mprotect: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit mprotect: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_capset(const struct event_t *e, const char *task_info) {
    syslog(LOG_INFO, "Enter capset: %s, header_version=%u, header_pid=%u, data_effective=%#x, data_permitted=%#x, data_inheritable=%#x\n",
...
static int handle_exit_capset(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit capset: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit capset: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_ptrace(const struct event_t *e, const char *task_info) {
    syslog(LOG_INFO, "Enter ptrace: %s, request=%d, pid=%u\n",
            task_info, e->arg_s32[0], e->arg_u32[0]);
    return 0;
}
...
static int handle_exit_ptrace(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit ptrace: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit ptrace: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_process_vm_readv(const struct event_t *e, const char *task_info) {
    syslog(LOG_INFO, "Enter process_vm_readv: %s, pid=%u, local_iov_len=%llu, remote_iov_len=%llu, flags=%#llx\n",
...
static int handle_exit_process_vm_readv(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit process_vm_readv: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit process_vm_readv: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_process_vm_writev(const struct event_t *e, const char *task_info) {
    syslog(LOG_INFO, "Enter process_vm_writev: %s, pid=%u, local_iov_len=%llu, remote_iov_len=%llu, flags=%#llx\n",
            task_info, e->arg_u32[0], e->arg_u64[0], e->arg_u64[1], e->arg_u64[2]);
    return 0;
}
...
static int handle_exit_process_vm_writev(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit process_vm_writev: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit process_vm_writev: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_init_module(const struct event_t *e, const char *task_info) {
    if (e->is_valid == true) {
        syslog(LOG_INFO, "Enter init_module: %s, len=%llu, param=%s\n",
                task_info, e->arg_u64[0], e->arg_str);
    } else {
        syslog(LOG_INFO, "Enter init_module: %s, len=%llu, failed to read param\n",
                task_info, e->arg_u64[0]);
    }
    return 0;
}
...
static int handle_exit_init_module(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit init_module: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit init_module: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_finit_module(const struct event_t *e, const char *task_info) {
    if (e->is_valid == true) {
        syslog(LOG_INFO, "Enter finit_module: %s, fd=%d, param=%s, flags=%#x\n",
                task_info, e->arg_s32[0], e->arg_str, e->arg_s32[1]);
    } else {
        syslog(LOG_INFO, "Enter finit_module: %s, fd=%d, failed to read param\n",
                task_info, e->arg_s32[0]);
    }
    return 0;
}
...
static int handle_exit_finit_module(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit finit_module: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit finit_module: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_delete_module(const struct event_t *e, const char *task_info) {
    syslog(LOG_INFO, "Enter delete_module: %s, name=%s, flags=%#x\n",
            task_info, e->arg_str, e->arg_s32[0]);
    return 0;
}
...
static int handle_exit_delete_module(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit delete_module: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit delete_module: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_munmap(const struct event_t *e, const char *task_info) {
    syslog(LOG_INFO, "Enter munmap: %s, addr=%#llx, len=%llu\n",
...
static int handle_exit_munmap(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit munmap: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit munmap: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_mremap(const struct event_t *e, const char *task_info) {
    syslog(LOG_INFO, "Enter mremap: %s, old_addr=%#llx, old_len=%llu, new_len=%llu, flags=%#x, new_addr=%#llx\n",
...
static int handle_exit_mremap(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit mremap: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit mremap: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_madvise(const struct event_t *e, const char *task_info) {
    syslog(LOG_INFO, "Enter madvise: %s, addr=%#llx, len=%llu, advice=%d\n",
...
static int handle_exit_madvise(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit madvise: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit madvise: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_mlock(const struct event_t *e, const char *task_info) {
    syslog(LOG_INFO, "Enter mlock: %s, addr=%#llx, len=%llu\n",
            task_info, e->arg_u64[0], e->arg_u64[1]);
    return 0;
}
...
static int handle_exit_mlock(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit mlock: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit mlock: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_mlock2(const struct event_t *e, const char *task_info) {
    syslog(LOG_INFO, "Enter mlock2: %s, addr=%#llx, len=%llu, flags=%#x\n",
            task_info, e->arg_u64[0], e->arg_u64[1], e->arg_s32[0]);
    return 0;
}
...
static int handle_exit_mlock2(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit mlock2: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit mlock2: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_munlock(const struct event_t *e, const char *task_info) {
    syslog(LOG_INFO, "Enter munlock: %s, addr=%#llx, len=%llu\n",
            task_info, e->arg_u64[0], e->arg_u64[1]);
    return 0;
}
...
static int handle_exit_munlock(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit munlock: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit munlock: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_mlockall(const struct event_t *e, const char *task_info) {
    syslog(LOG_INFO, "Enter mlockall: %s, flags=%#x\n",
            task_info, e->arg_s32[0]);
    return 0;
}
...
static int handle_exit_mlockall(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit mlockall: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit mlockall: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_munlockall(const struct event_t *e, const char *task_info) {
    syslog(LOG_INFO, "Enter munlockall: %s\n", task_info);
    return 0;
}

static int handle_exit_munlockall(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit munlockall: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit munlockall: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_mincore(const struct event_t *e, const char *task_info) {
    syslog(LOG_INFO, "Enter mincore: %s, addr=%#llx, len=%llu\n",
            task_info, e->arg_u64[0], e->arg_u64[1]);
    return 0;
}
...
static int handle_exit_mincore(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit mincore: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit mincore: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_membarrier(const struct event_t *e, const char *task_info) {
    syslog(LOG_INFO, "Enter membarrier: %s, cmd=%d, flags=%#x, cpu_id=%d\n", 
            task_info, e->arg_s32[0], e->arg_u32[0], e->arg_s32[1]);
    return 0;
}
...
static int handle_exit_membarrier(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit membarrier: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit membarrier: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_capget(const struct event_t *e, const char *task_info) {
    syslog(LOG_INFO, "Enter capget: %s\n", task_info);
    return 0;
}

static int handle_exit_capget(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit capget: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else if (e->is_valid == true) {
        syslog(LOG_INFO, "Enter capget: success, %s, header_version=%u, header_pid=%u, data_effective=%#x, data_permitted=%#x, data_inheritable=%#x, ret=%lld\n",
            task_info, e->arg_u32[0], e->arg_u32[1], e->arg_u32[2], e->arg_u32[3], e->arg_u32[4], e->ret);
    } else {
        syslog(LOG_INFO, "Exit capget: success, %s, failed to read cap values, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_prctl(const struct event_t *e, const char *task_info) {
    syslog(LOG_INFO, "Enter prctl: %s, option=%d\n",
            task_info, e->arg_s32[0]);
    return 0;
}
...
static int handle_exit_prctl(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit prctl: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit prctl: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_arch_prctl(const struct event_t *e, const char *task_info) {
    syslog(LOG_INFO, "Enter arch_prctl: %s, option=%d, addr=%#llx\n",
            task_info, e->arg_s32[0], e->arg_u64[0]);
    return 0;
}
...
static int handle_exit_arch_prctl(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit arch_prctl: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit arch_prctl: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_kill(const struct event_t *e, const char *task_info) {
    syslog(LOG_INFO, "Enter kill: %s, pid=%u, sig=%d\n",
            task_info, e->arg_u32[0], e->arg_s32[0]);
    return 0;
}
...
static int handle_exit_kill(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit kill: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit kill: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_tkill(const struct event_t *e, const char *task_info) {
    syslog(LOG_INFO, "Enter tkill: %s, tid=%u, sig=%d\n",
            task_info, e->arg_u32[0], e->arg_s32[0]);
    return 0;
}
...
static int handle_exit_tkill(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit tkill: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit tkill: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
...
static int handle_enter_tgkill(const struct event_t *e, const char *task_info) {
    syslog(LOG_INFO, "Enter tgkill: %s, tgid=%u, tid=%u, sig=%d\n",
...
static int handle_exit_tgkill(const struct event_t *e, const char *task_info) {
    if (e->ret < 0) {
        syslog(LOG_INFO, "Exit tgkill: failed, %s, error_code=%lld\n",
                task_info, e->ret);
    } else {
        syslog(LOG_INFO, "Exit tgkill: success, %s, ret=%lld\n",
                task_info, e->ret);
    }
    return 0;
}
